...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 且 `runtime.performance.enabled=true` 时，listener 按 `sched_getaffinity` 检测到的可用 CPU 核建立 `SO_REUSEPORT` accept worker；plain HTTP 保留每核 accept fanout，但已建立连接进入同一个按 cpuset CPU 数扩展的 work-stealing Tokio runtime，避免 SO_REUSEPORT 的稳定 hash 在 mixed keep-alive/SSE/file 波次里把热点永久锁在单线程 shard。游戏小帧、通用 TCP 与 plain WebSocket 交给按 cpuset 线性扩展的原生 `epoll` relay reactor：实时优先的 `small` 与默认混合负载 `balanced` profile 每 2 个可用 CPU 一个 owner，纯大流量的 `bulk` 每 4 个可用 CPU 一个 owner，为文件/HTTP 保留更多 scheduler 预算。每条连接只由一个 reactor owner 维护 backpressure 与 half-close，避免每帧跨 Tokio 调度；`EPOLLHUP`/`EPOLLRDHUP` 与 `EPOLLIN` 同时出现时先 drain 尾部数据再传播 half-close，不能丢弃游戏/WebSocket 最后一帧。每 worker 不超过 4 对低密度连接时只做极短 reply spin，中高密度完全依赖 `epoll` batch；不可 handoff 或非 Linux 时回退到有界 Tokio relay，bulk 流保留 `splice` 路径。UDP 同样按 CPU 自适应 fanout，并共享 association 表。
...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 accept worker 承载；worker 数以 `sched_getaffinity` 的 cpuset 为准，不设固定小上限。plain HTTP accept fanout 后进入 CPU 比例的共享 work-stealing runtime，使不均匀的 keep-alive/SSE/静态连接能迁移到空闲 worker，而不是被 SO_REUSEPORT hash 固定成单核热点。plain `ws://` 与小帧 TCP 进入原生 `epoll` relay：实时优先 `small` 与默认混合负载 `balanced` profile 每 2 个可用 CPU 一个 owner，纯大流量 `bulk` 每 4 个 CPU 一个 owner，避免单 reactor 和每帧 async 调度瓶颈，并按 traffic profile 给文件/HTTP/TLS/UDP 保留调度空间；WSS 因 rustls 语义保留多核 Tokio 有界 relay。正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标，内存以 current/peak/每连接成本报告，只有实际生产预算才设上限。TLS 使用独立的 CPU 比例 work-stealing runtime；达到每 base shard 64 条活跃 TLS 连接时，额外 TLS accept shard 才弹性启动。

...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder，不再走 HTTP/1 自动探测；TLS accept、handshake 与 H2 connection 全程留在同一个 shard，避免跨 runtime 搬运和 wake。`SO_REUSEPORT` 把不同连接分散到 CPU 自适应 shard。小静态文件进入有界内存缓存，H2 热对象以 immutable `Bytes` 直接应答，并使用 stale-while-revalidate。Range 精确返回 `206`/`416`；大文件 Linux fast lane 使用缓存 fd + `sendfile`，单次 chunk 随 traffic profile 自适应为 `small=2 MiB`、`balanced=8 MiB`、`bulk=16 MiB`。同一次 writable readiness 内连续 drain partial writes，直到 chunk 预算、EOF 或 `EAGAIN`，避免每次正向 partial write 都重新进入 Tokio readiness future。默认 `small` 做 125µs 公平 pacing；`balanced` 不做固定 sleep，也不切断单个大响应，而是在每个 sendfile 响应完成、读取下一条 keep-alive 请求前 cooperative yield 一次，兼顾连续零拷贝与共享 HTTP shard 小包公平。要求同一轮把 small/realtime 与 bulk 都设为严格 gate 时，可显式使用 `TRAFFIC_PROFILE=balanced`，但必须重新通过全部混合场景，不能只报大文件单项。

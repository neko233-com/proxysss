...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载；worker 数以 `sched_getaffinity` 的 cpuset 为准，不设固定小上限。plain `ws://` 与小帧 TCP 进入原生 `epoll` relay：`small` 每 2 个可用 CPU 一个 owner，默认混合负载 `balanced` 与 `bulk` 每 4 个 CPU 一个 owner；所有 owner 都随 cpuset 线性增长，空闲时阻塞。WSS 因 rustls 语义保留 Tokio 有界 relay。默认 `balanced` 让 plain HTTP、TCP accept、UDP recv/association 共用 per-core runtime shard，worker index 决定 CPU owner；TLS 使用 `ceil(cpuset cores / 4)`、nice +3 的独立 work-stealing crypto runtime，避免计算密集握手/H2 与 UDP/HTTP 小包互相形成 head-of-line blocking。正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标，内存以 current/peak/每连接成本报告，只有实际生产预算才设上限。达到每 base shard 64 条活跃 TLS 连接时，额外 TLS accept shard 才弹性启动。

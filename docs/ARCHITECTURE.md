...
- Linux `runtime.performance.enabled=true` 下，plain HTTP accept/general request 与 balanced UDP 复用每核一个 nice 0 shard；首次完整校验后的 exact static keep-alive connection 转交每 cpuset CPU 一个原生 `epoll` owner，避免 Tokio readiness/task 调度成本。小对象用合并 head/body 的单次 `send`，大对象用 `TCP_CORK` + `sendfile`；每 event 最多处理 32 个响应后重新进入 epoll 公平调度。文件 metadata 校验按 owner + path 合并为最多每秒一次，避免连接数放大同步 stat。配置通过 ArcSwap 原子发布并递增 epoch；epoch、请求字节或文件 metadata 变化立即把 socket 与未消费 prefix 无损交回 Tokio/Hyper。UDP 热关联每 8 包让出一次，realtime reactor 每 4 CPU 一个 nice +5 owner并在连续 8 个 ready batch 后让出。TLS 使用 `ceil(cpuset cores / 8)`、nice 0 的独立 runtime。

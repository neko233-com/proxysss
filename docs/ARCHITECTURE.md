...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 且 `runtime.performance.enabled=true` 时，listener 按 `sched_getaffinity` 检测到的可用 CPU 核建立 `SO_REUSEPORT` accept worker；游戏小帧、通用 TCP 与 plain WebSocket 交给按 cpuset 线性扩展的原生 `epoll` relay reactor：`small` 每 2 个可用 CPU 一个 owner，默认混合负载 `balanced` 与 `bulk` 每 4 个 CPU 一个 owner。balanced owner 使用 nice +3，空闲时阻塞并保留显式 backpressure、half-close 与尾帧 drain。默认 `balanced` 让 UDP recv/association 与 plain HTTP 复用对应的 per-core runtime shard，避免额外线程与跨 runtime wake；TLS 使用 `ceil(cpuset cores / 4)`、nice +1 的独立有界 crypto runtime。大文件交给 `ceil(cpuset cores / 4)`、nice +3 的 sendfile reactor，避免阻塞 HTTP owner。
...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载。plain HTTP 和默认 balanced UDP recv/association 复用每核一个 nice 0 runtime shard；connection-local runtime 每 31 次 task poll 检查 global queue、每 16 次驱动 I/O/timer。TLS 使用 `ceil(cpuset cores / 4)`、nice +1 的独立 crypto runtime，以较少 owner 降低 CFS 竞争并保持空闲 CPU 可用。plain `ws://` 与小帧 TCP 的 balanced reactor 每 4 CPU 一个 nice +3 owner。正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标。
...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder；TLS accept、handshake 与 H2 connection 全程留在 `ceil(cpuset cores / 4)`、nice +1 的独立 crypto runtime。H2 小静态热对象以 immutable `Bytes` 应答并用 stale-while-revalidate。大文件复用缓存 fd 与 Linux `sendfile`；balanced 与 bulk 都使用原生 epoll sendfile reactor，其中 balanced 为 `ceil(cpuset cores / 4)`、nice +3，bulk 为全核 nice 0。每响应使用独立 offset，handoff 失败回退 Tokio readiness；small 保留 125µs pacing。所有 profile 变更都必须重跑完整 mixed gate。
...
- 60 秒 validation deadline 会下传到每个 client wave，并由 GNU `timeout` 按剩余预算硬终止；超时同时停止容器内发生器并失败，不再只在矩阵结束后检查。默认统一启动 lead 为 100 ms，UDP/QCP 尾部等待 1 秒，3 秒测量窗口不缩短。

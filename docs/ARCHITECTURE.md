...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder；默认 `small` 的 TLS handshake、H2 与 plain HTTP 共用相同 per-core data shard，避免 crypto runtime 额外抢占，`balanced` TLS 仍使用每 8 核一个 bounded owner。H2 小静态热对象使用 immutable `Bytes` 与 lock-free 配置快照。small 大文件使用 16 MiB `sendfile` ceiling，通常由 socket `WouldBlock` 自然提供背压与让出；只有完整 chunk 仍立即可写时才 cooperative yield，`bulk` 才启用全核 reactor。release 使用 fat LTO 与单 codegen unit。所有 profile 变更都必须重跑完整 mixed gate。

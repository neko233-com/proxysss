...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 且 `runtime.performance.enabled=true` 时，listener 按 cpuset 建立 `SO_REUSEPORT` worker；游戏小帧、TCP 与 plain WebSocket 的 balanced 原生 `epoll` relay 每 4 CPU 一个 nice +3 owner。默认 `balanced` 让 UDP recv/association 与 plain HTTP 复用对应的 per-core runtime shard；TLS 使用 `ceil(cpuset cores / 4)`、nice +3 的独立 crypto runtime。运行配置快照通过 ArcSwap 原子替换，数据面读取不再进入异步 `RwLock`；reload 构建完整新状态后一次 publish。balanced 大文件在 HTTP owner 内使用 writable readiness + `sendfile`，bulk 才启用全核 nice 0 sendfile reactor。
...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载。plain HTTP 和 balanced UDP 复用每核一个 nice 0 runtime shard；TLS 使用 `ceil(cpuset cores / 4)`、nice +3 的独立 crypto runtime；plain `ws://` 与小帧 TCP 的 balanced reactor每 4 CPU 一个 nice +3 owner。配置 hot reload 先构建完整 `DynamicState`，再通过 ArcSwap 原子发布；请求读取 lock-free `Arc` 快照，旧请求安全持有旧快照直到结束。正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标。
...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder；TLS 全程留在 `ceil(cpuset cores / 4)`、nice +3 的 crypto runtime。H2 小静态热对象使用 immutable `Bytes` 与 lock-free 配置快照。大文件复用缓存 fd 与 Linux `sendfile`；balanced 在固定 HTTP owner 的 writable readiness 上 drain，bulk 启用全核 nice 0 epoll sendfile reactor。每响应使用独立 offset，handoff 失败回退 Tokio readiness。所有 profile 变更都必须重跑完整 mixed gate。

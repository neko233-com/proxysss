...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 性能模式从 cgroup effective cpuset（而非可能已被 pin 的当前线程）发现完整 CPU 集，并据此建立全核 `SO_REUSEPORT` listener fanout。默认 `small` 把 HTTP/1.1、HTTP/2/TLS、WebSocket、TCP、UDP 与透明 QCP 放进同一组 CPU 自适应 per-core data shard；`balanced` 把 TLS/H2 放到 nice 5 bounded runtime，把 UDP/QCP 放到 nice 3 bounded runtime，并在透明 TCP/plain WebSocket 完成 connect/upgrade 后交给 sparse nice 0 native epoll relay，HTTP ready queue 只保留 HTTP/SSE/reverse 工作。`bulk` 可启用独立 transfer owner/sendfile reactor。配置通过 ArcSwap 原子发布。

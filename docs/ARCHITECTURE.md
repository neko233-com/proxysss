...
- Linux `runtime.performance.enabled=true` 下，plain HTTP 使用每核一个 pinned nice 0 shard。balanced UDP 使用 `ceil(cpuset cores / 4)`、nice +3 的独立 runtime，避免高吞吐 UDP/QCP 抢占 HTTP owner，同时在低负载空闲 CPU 上仍可立即运行；UDP 热关联每 8 包让出一次。realtime reactor 每 4 CPU 一个 nice +5 owner并在连续 8 个 ready batch 后让出。TLS 使用 `ceil(cpuset cores / 8)`、nice 0 的独立 runtime。plain fast lane 在少于 300 活跃连接时每 8 响应让出，300 以上每 32 响应让出。配置 hot reload 通过 ArcSwap 原子发布。

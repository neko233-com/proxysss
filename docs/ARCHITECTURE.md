...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder，不再走 HTTP/1 自动探测；TLS accept、handshake 与 H2 connection 全程留在同一个 shard，避免跨 runtime 搬运和 wake。`SO_REUSEPORT` 把不同连接分散到 CPU 自适应 shard。小静态文件进入有界内存缓存，H2 热对象以 immutable `Bytes` 直接应答，并使用 stale-while-revalidate。Range 精确返回 `206`/`416`；大文件 Linux fast lane 使用缓存 fd + `sendfile`，单次 chunk 随 traffic profile 自适应为 `small=2 MiB`、`balanced=8 MiB`、`bulk=16 MiB`。默认 `balanced` 由当前 CPU-adaptive HTTP connection owner 直接在 Tokio writable readiness 上连续 drain partial writes，直到 chunk 预算、EOF 或 `EAGAIN`，避免每响应 duplicate fd、eventfd、epoll add/del 与 oneshot 往返；完整响应后、读取下一条 keep-alive 请求前 cooperative yield 一次。明确选择 `bulk` 时才把 duplicated socket/file fd 交给有界、每核分片、round-robin 的原生 epoll sendfile reactor，隔离长时间 writable drain；`sendfile(2)` 使用每响应独立的显式 offset，queue 满或 handoff 失败时从当前 offset 回退 Tokio readiness。默认 `small` 仍做 125µs 公平 pacing。要求同一轮把 small/realtime 与 bulk 都设为严格 gate 时，可显式使用 `TRAFFIC_PROFILE=balanced`，但必须重新通过全部混合场景，不能只报大文件单项。

...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 性能模式从 cgroup effective cpuset（而非可能已被 pin 的当前线程）发现完整 CPU 集，并据此建立全核 `SO_REUSEPORT` listener fanout。默认 `small` 把 HTTP/1.1、HTTP/2/TLS、WebSocket、TCP、UDP 与透明 QCP 放进同一组 CPU 自适应 per-core data shard；`balanced` 把 TLS/H2 放到 nice 5 bounded runtime，把 UDP/QCP 放到 `ceil(cores/2)` nice 3 runtime，并在透明 TCP/plain WebSocket 完成 connect/upgrade 后交给 `ceil(cores/2)` nice 3 native epoll relay，HTTP ready queue 只保留 HTTP/SSE/reverse 工作。`bulk` 可启用独立 transfer owner/sendfile reactor。配置通过 ArcSwap 原子发布。
...
- Linux `runtime.performance.enabled=true` 下，从 cgroup effective cpuset 发现完整 CPU 集；默认 `small` 的 HTTP/1.1、HTTP/2/TLS、WebSocket、TCP、UDP 与透明 QCP 共用一个 CPU-sized Tokio runtime，每个允许核心一个 nice 0 worker，并允许 ready connection 在这些 per-core shard 之间 work-steal。`balanced` 的 TLS/H2 使用 nice 5 bounded runtime；UDP/QCP 使用 `ceil(cpuset/2)` nice 3 runtime；policy-free realtime TCP 和 plain WebSocket 在 connect/upgrade 后进入 `ceil(cpuset/2)` nice 3 native epoll relay。这样移除 HTTP ready queue 内的长期 relay future，同时让 HTTP 在 saturation 下保持更高 CFS 权重。`ceil(cpuset/4)` nice 0 的实验在 1×/4× 将 TCP/WebSocket 压到 0.85-0.98x nginx，并恶化 p95/p99，已拒绝。统一 balanced TLS 的实测会产生 16-21 ms 跨协议尾峰并把 HTTPS saturation 压到最低 0.225x，因此不得重新合并。Tokio LIFO slot 保留。global injection queue 每 31 次 poll、I/O driver 每 16 次 poll 检查；8-poll 实验降低了 4x HTTP 吞吐且没有修复尾延迟。plain static、reverse proxy、SSE 仅在单次有界 socket exchange 内关闭 Tokio 隐式 cooperative counter，连接仍每 256 响应显式 yield，避免 128 次 I/O 后统一形成约 2 ms 尾峰，又不会让热 keep-alive 无限运行。balanced 专用 UDP recv loop 仅在 8 ms 内持续收到 32 包后让出，确保 association response reader 可运行；shared-small UDP 保留 4 包边界。静态文件使用 2 秒 freshness window，metadata/body/H2 snapshot 重验证进入单独 nice-10 control runtime，不占 data shard。small-profile WebSocket 使用 readiness-bounded 64-poll normal relay turn。`bulk` 使用专用 transfer owner。HTTP 请求统计使用每 data worker 独占的 cache-line padded counter，采集时汇总；配置 hot reload 通过 ArcSwap 原子发布。

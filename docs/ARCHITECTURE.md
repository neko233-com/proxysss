...
7. Proxy request/response (including WebSocket upgrade, generic SSE streaming, New API-compatible routes, and gRPC-over-h2), or serve static files with bounded memory cache, mmap-backed hot objects on supported builds, Range/206 resumable downloads, and a Linux plain-HTTP fast lane whose cache/sendfile behavior follows `runtime.performance.traffic_profile`. After the first policy-validated exact static GET, performance mode hands the keep-alive socket to per-cpuset native `epoll` owners: small objects use one combined `send`, large objects use `TCP_CORK` plus `sendfile`, and any request, file metadata, or configuration-epoch change returns the socket and unread prefix to Tokio/Hyper without dropping bytes.
...
- Linux `runtime.performance.enabled=true` 下，从 cgroup effective cpuset 发现完整 CPU 集；plain HTTP accept/general request 与 balanced UDP 复用每核一个 nice 0 shard。首次完整校验后的 exact static keep-alive connection 转交对应 HTTP shard 的原生 `epoll` owner；后续请求按已验证 head 长度直接做 byte equality，普通非 pipeline 命中直接清空复用 input buffer，不再搜索 delimiter 或执行 `Vec::drain`。小对象用合并 head/body 的单次 `send`，普通成功响应保持稳定 `EPOLLIN`，只有真实写 backpressure 或 32-response 公平续跑才 `epoll_ctl` 切换 `EPOLLOUT`；大对象用 `TCP_CORK` + `sendfile`，文件 metadata 校验按 owner + path 合并为最多每秒一次。配置通过 ArcSwap 原子发布并递增 epoch；epoch、请求字节或文件 metadata 变化立即把 socket 与未消费 prefix 无损交回 Tokio/Hyper。UDP 只在 8 包/8ms 的持续 burst 下让出；realtime reactor 每 4 CPU 一个 nice +5 owner并在连续 8 个 ready batch 后让出；TLS 使用 `ceil(cpuset cores / 8)`、nice 0 的独立 runtime。普通 plain fast lane 在少于 300 活跃连接时每 8 响应让出，300 以上每 32 响应让出。

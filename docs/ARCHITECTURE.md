...
- 上述 32 次 nonblocking poll 与后续 timed-wait 都是已拒绝实验，不是当前实现。当前 1-16/17-32/33-128 对 owner 分别使用 1/2/8 次 immediate probe；8 次在跨过 backend scheduling gap 的同时限制 CPU 占用，kernel sleep/wake 未改善 realtime tail。adaptive sendfile 在 active transfer 不超过每数据核 2 条时使用 8 MiB event budget，让 1× 大文件交错推进；更高压力保留 full-response budget 与自然 `EAGAIN`。reactor 提前清除 `TCP_CORK` 后会把状态传回 HTTP owner，避免 owner 重复 syscall。

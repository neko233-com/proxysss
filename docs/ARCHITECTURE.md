...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载；worker 数以 `sched_getaffinity` 的 cpuset 为准，不设固定小上限。plain `ws://` 与小帧 TCP 默认进入每 2 个可用 CPU 一个 owner 的原生 `epoll` relay，避免单 reactor 和每帧 async 调度瓶颈，同时给 HTTP/TLS/UDP 保留混合负载调度空间；WSS 因 rustls 语义保留多核 Tokio 有界 relay。正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标，内存以 current/peak/每连接成本报告，只有实际生产预算才设上限。TLS 低密度连接复用 HTTP shard；达到每 base shard 64 条活跃 TLS 连接时，额外 TLS shard 才弹性启动。
...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder，不再走 HTTP/1 自动探测；TLS accept、handshake 与 H2 connection 全程留在同一个 shard，避免跨 runtime 搬运和 wake。`SO_REUSEPORT` 把不同连接分散到 CPU 自适应 shard。小静态文件进入有界内存缓存，H2 热对象以 immutable `Bytes` 直接应答，并使用 stale-while-revalidate。Range 精确返回 `206`/`416`；大文件 Linux fast lane 使用缓存 fd + `sendfile`，单次 chunk 随 traffic profile 自适应为 `small=2 MiB`、`balanced=8 MiB`、`bulk=16 MiB`。`balanced`/`bulk` 把 duplicated socket/file fd 交给有界、每核分片、round-robin 的原生 epoll sendfile reactor；HTTP shard 保留连接与 keep-alive 所有权并等待 completion，因此 bulk writable drain 不再占住通用 HTTP scheduler。同一次 EPOLLOUT 内连续 drain partial writes，直到 chunk 预算、EOF 或 `EAGAIN`；queue 满或 handoff 失败时回退 Tokio readiness。默认 `small` 仍做 125µs 公平 pacing；`balanced` 在完整 sendfile 响应后、读取下一条 keep-alive 请求前 cooperative yield 一次。要求同一轮把 small/realtime 与 bulk 都设为严格 gate 时，可显式使用 `TRAFFIC_PROFILE=balanced`，但必须重新通过全部混合场景，不能只报大文件单项。

...
- TCP stream 有独立 relay profile。Linux 性能模式下，游戏小帧与通用 1 KiB realtime TCP 默认使用 CPU 自适应分片的原生 `epoll` relay，每条连接由单一 reactor owner 维护显式 backpressure 和 half-close；`small` 与 `balanced` traffic profile 每 2 个 cpuset CPU 一个 owner，`bulk` 每 4 个 CPU 一个 owner，把足够调度时间留给并发文件/HTTP/TLS/UDP，二者都随高核 cpuset 线性增长而不设固定上限。单 owner 达到 32 对 socket 的 mixed-load 密度后，每个已处理 epoll batch 主动让出一次 CPU，使同核 HTTP accept shard 获得确定调度点；稀疏 realtime 连接不走这条公平分支。handoff 不可用或非 Linux 时使用多核 Tokio 双向 relay 与有界 `ByteBufferPool`，bulk/file/backup profile 才使用 `splice(socket -> pipe -> socket)`。

...
For a normal public WSS hostname, `http.tls.auto_https.domains: [wss.example.com]` is a domain-only bootstrap: a non-empty list selects embedded managed ACME, production Let's Encrypt, and HTTP-01. No external certificate client, DNS API credential, or account email is required. The gateway owns `/.well-known/acme-challenge/*`, certificate persistence, renewal, and reload; the public DNS record must resolve to the gateway and ports 80/443 must be reachable. `email` is intentionally optional and only enables expiry/security notices. Explicit managed HTTP-01, TLS-ALPN-01, DNS-01, and legacy external ACME forms remain supported.
...
- TCP stream proxying has its own execution pool and relay profile selector instead of sharing HTTP/static tuning. With `runtime.performance.enabled=true`, TCP listeners run on a dedicated `proxysss-stream` Tokio runtime so long-lived connections are not starved by HTTP/static/SSE workloads. The default latency profile uses 16KB relay buffers and a bounded full-duplex state machine that preserves half-close behavior for request/response long connections; the bulk profile is reserved for file/backup-style streams and can use Linux `splice(socket -> pipe -> socket)` to avoid user-space data copies when it is actually beneficial.
- WebSocket upgrades use the same full-duplex relay discipline: both the native `ws://` lane and the generic HTTP/TLS upgrade tunnel borrow two 4KiB game-frame buffers from a bounded lock-free pool, preserve TCP half-close behavior, and return surplus buffers after a reconnect wave instead of allocating two fresh heap buffers for every tunnel. The relay consumes Tokio cooperative budget and limits a WebSocket direction to one read/write batch per worker turn, so a hot peer cannot starve thousands of game sessions. This keeps 100k mostly-idle tunnels within an 8GiB gateway memory budget instead of pinning 3.2GiB in 16KiB relay buffers alone. HTTP/WebSocket accept and upstream sockets keep Linux kernel-default queue sizing plus `TCP_NODELAY`: applying multi-megabyte `setsockopt` buffers to every upgraded socket would add handshake syscalls and make high connection counts memory-hostile. Direct realtime TCP listeners still use the 512KiB/1MiB queue target and 16KiB `TCP_NOTSENT_LOWAT` profile. This keeps game-client reconnect churn and slow-peer queueing out of the allocator/data-path hot path while leaving WSS/TLS protocol handling unchanged.
- Static files below the in-memory threshold use a bounded in-process cache with short mtime/size revalidation for hot objects; large cached objects use mmap-backed `Bytes` where available to reduce heap allocation and copy pressure. Range requests bypass full-body cache responses and return `206 Partial Content`/`416 Range Not Satisfiable` with byte-accurate `Content-Range`, streaming large ranges instead of buffering them. `runtime.performance.traffic_profile` controls the static fast-lane tradeoff: default `small` favors cached small-file/HTTP2/SSE/TCP/UDP feedback, `bulk` favors Linux sendfile/zero-copy large transfers, and `balanced` prepares both. Config load preloads eligible static index/top-level files into the bounded cache or opens sendfile descriptors according to that profile.

...
- Linux `runtime.performance.enabled=true` 下，从 cgroup effective cpuset 发现完整 CPU 集；plain HTTP 与 balanced UDP 复用每核一个 nice 0 shard，balanced UDP 只在 8 包/8ms 的持续 burst 下让出，默认 small UDP/QCP 使用独立 nice +2 runtime；realtime reactor 每 4 CPU 一个 nice +5 owner并在连续 8 个 ready batch 后让出。默认 small TLS 使用 `ceil(cpuset cores / 4)`，balanced 使用 `ceil(cpuset cores / 8)`，均为 nice 0 独立 runtime。plain fast lane 在少于 300 活跃连接时每 8 响应让出，300 以上每 32 响应让出。配置 hot reload 通过 ArcSwap 原子发布。

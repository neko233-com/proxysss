...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 性能模式从 cgroup effective cpuset（而非可能已被 pin 的当前线程）发现完整 CPU 集，并据此建立全核 `SO_REUSEPORT` listener fanout。默认 `small` 与 `balanced` 把 HTTP/1.1、HTTP/2/TLS、WebSocket、TCP、UDP 与透明 QCP 放进同一组 CPU 自适应 per-core data shard，避免额外 TLS/UDP runtime 与 HTTP 争抢 CFS 时间；持续 UDP burst 每 4 个数据报 cooperative yield。`bulk` 才启用独立 transfer owner/sendfile reactor。配置通过 ArcSwap 原子发布。

...
- `scripts/benchmark-all-scenarios-isolated.sh` 默认每尺度对每个 gateway/phase 运行 1 个同步 1 秒样本；完整 1×/2×/4× active measurement 为 12 秒并硬限制在 20 秒内，build/setup/warm-up 不计入。双方使用相同端口、cpuset、nofile、somaxconn、共享 sysctl、协议面与场景 CPU 分区。equal-load 取较慢 saturation 的 25%，要求至少完成 98% 可执行 target 且 proxysss 每个 percentile 严格更低。HTTP/SSE 使用 centered uniform phase 并对 aggregate slot 四舍五入；WebSocket/TCP/UDP 使用严格早于 deadline 的 whole per-connection synchronized ticks。nginx 1.31.2 mainline 使用 `-O3 -fno-plt`、相同 gateway cpuset 和等价配置。透明 QCP 只证明 edge forwarding，不声称 frame termination。serial isolated saturation 不能替代 mixed gate；Docker role isolation 不能冒充三台物理机。

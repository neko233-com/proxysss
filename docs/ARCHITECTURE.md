...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载；worker 数以 `sched_getaffinity` 的 cpuset 为准，不设固定小上限。默认 `balanced` 让 plain HTTP、plain `ws://`、TCP accept/relay 与 UDP recv/association 共用 per-core Tokio runtime shard，worker index 决定 CPU owner；connection-local data runtime 以 31 次 task poll 摊销一次 global injection queue 检查、以 8 次 task poll 摊销一次 I/O/timer driver 检查，并把 HTTP/TCP/UDP 单次 driver tick 限制为 128 个 readiness event；TLS/H2 保留 256，避免 crypto runtime 因过度轮转损失吞吐。这样既避免高 I/O wave 长时间占住 event loop，也保护 HTTPS/realtime p95/p99。WSS 因 rustls 语义保留 Tokio 有界 relay。TLS 使用 `ceil(cpuset cores / 2)`、nice +7 的独立 work-stealing crypto runtime，在高核主机上线性扩展，同时限制混合负载中的 TLS CPU 份额并避免固定热点核。正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标，内存以 current/peak/每连接成本报告，只有实际生产预算才设上限。达到每 base shard 64 条活跃 TLS 连接时，额外 TLS accept shard 才弹性启动。

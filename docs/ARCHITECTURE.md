...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder，不再走 HTTP/1 自动探测；TLS accept、handshake 与 H2 connection 全程留在同一个 shard，避免跨 runtime 搬运和 wake。`SO_REUSEPORT` 把不同连接分散到 CPU 自适应 shard。小静态文件进入有界内存缓存，H2 热对象以 immutable `Bytes` 直接应答，并使用 stale-while-revalidate。Range 精确返回 `206`/`416`；大文件 Linux fast lane 使用缓存 fd + `sendfile`，单次 chunk 随 traffic profile 自适应为 `small=2 MiB`、`balanced=16 MiB`、`bulk=16 MiB`。`balanced`/`bulk` 把 duplicated socket/file fd 交给每核一个、随 cpuset 线性扩展的有界原生 epoll sendfile reactor；worker 绑定自己的 CPU group，handoff 按当前 HTTP shard 所在 CPU 稳定映射，避免 response 之间来回迁移。HTTP shard 保留连接与 keep-alive 所有权并等待 completion，因此 bulk writable drain 不再占住通用 HTTP scheduler。同一次 EPOLLOUT 内连续 drain partial writes，直到 chunk 预算、EOF 或 `EAGAIN`；queue 满或 handoff 失败时回退 Tokio readiness。默认 `small` 仍做 125µs 公平 pacing；native reactor completion 后立即读取下一条 keep-alive 请求，不再重复 cooperative yield，普通 fast lane 仍每 32 个响应公平让出。要求同一轮把 small/realtime 与 bulk 都设为严格 gate 时，可显式使用 `TRAFFIC_PROFILE=balanced`，但必须重新通过全部混合场景，不能只报大文件单项。

...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 且 `runtime.performance.enabled=true` 时，listener 按 `sched_getaffinity` 检测到的可用 CPU 核建立 `SO_REUSEPORT` accept worker；游戏小帧、通用 TCP 与 plain WebSocket 默认使用多核 Tokio relay，bulk 流保留 `splice` 路径。原生 `epoll` reactor 代码保留为高连接密度实验路径，但不会取代已验证的低连接高 tick 多核路径。UDP 同样按 CPU 自适应 fanout，并共享 association 表。
...
4. TCP 默认关闭 Nagle（`nodelay: true`）并应用 `connect_timeout_ms`。游戏、MQTT/tool 与通用实时流使用有界 buffer pool 的 Tokio 双向 relay，并依赖 Linux socket autotuning；明确的 bulk/file 协议才使用带 `SPLICE_F_MORE` 的 Linux `splice` 零拷贝。
...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载；worker 数以 `sched_getaffinity` 的 cpuset 为准，不设固定小上限。plain `ws://`、游戏小帧 TCP 与 WSS 都使用多核 Tokio relay，避免单 reactor 唤醒瓶颈；正式 4c/8GiB 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标。TLS 低密度连接复用 HTTP shard；达到每 base shard 64 条活跃 TLS 连接时，额外 TLS shard 才弹性启动。WSS 始终保留 rustls/AWS-LC 语义。
...
- TCP stream 有独立 relay profile。游戏小帧与通用 1 KiB realtime TCP 使用多核 Tokio 双向 relay、有界 `ByteBufferPool`、显式 backpressure 和 half-close；bulk/file/backup profile 才使用 `splice(socket -> pipe -> socket)`。
- plain WebSocket 在 HTTP shard 完成握手后继续使用多核 Tokio relay；WSS 使用同样的有界 relay 加 rustls/AWS-LC。空闲长连接不会预占每方向固定大 buffer。4c/8GiB 的生产容量门禁默认验证 20k idle WSS，活跃消息规模验证到 4096，并分别报告握手与消息 p50/p95/p99。
...
- `scripts/benchmark-all-scenarios-isolated.sh` 是 4c/8GiB 单网关的严格 Linux 对照入口：gateway、backend、client 使用互不重叠的 cpuset/cgroup 和独立容器网络命名空间，默认 4+4+8 CPU 分配先做 16-core 预检；nginx 1.31.2 mainline 与 proxysss 接受同一 workload。mixed saturation、equal-offered-load 各默认交错运行 4 次，serial isolated saturation 运行 3 次并按指标取中位数，任一轮错误仍保留最大值；equal load 使用较慢方饱和吞吐的固定比例严格判 p50/p95/p99 与完成率。`MIXED_SCENARIOS` 可做不改变方法学的单项诊断。`benchmark-websocket-production-gate.sh` 另测多尺度 WSS active 与 20k idle hold；`benchmark-cross-host-wss.sh` 从独立 client host 把同 SHA binary 布置到 gateway/backend，并以远端 systemd cgroup 强制 4 CPU、8 GiB、300k nofile，保留 cgroup memory current/peak、主机/`nginx -V` 指纹及原始样本，再严格复跑 WSS 吞吐、p50/p95/p99 和容量。Docker role isolation 不能冒充三台物理机。

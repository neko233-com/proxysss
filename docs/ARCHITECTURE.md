...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载；worker 数以 `sched_getaffinity` 的 cpuset 为准，不设固定小上限。plain `ws://` 与小帧 TCP 进入原生 `epoll` relay：`small` 每 2 个可用 CPU 一个 owner，默认混合负载 `balanced` 与 `bulk` 每 4 个 CPU 一个 owner；所有 owner 都随 cpuset 线性增长，空闲时阻塞。WSS 因 rustls 语义保留 Tokio 有界 relay。默认 `balanced` 的 plain HTTP 保持每核一个 nice 0 runtime shard；UDP recv/association 使用 `ceil(cpuset cores / 4)`、nice +3 的独立 runtime，避免高包率 datagram 与 HTTP task 在同一 Tokio queue 争抢；connection-local data runtime 以 31 次 task poll 摊销一次 global injection queue 检查、以 16 次 task poll 摊销一次 I/O/timer driver 检查，避免旧 2/3 间隔把小包 CPU 消耗在 scheduler bookkeeping，同时仍比 Tokio 默认吞吐节奏更频繁地驱动 I/O。TLS 使用 `ceil(cpuset cores / 2)`、nice +3 的独立 work-stealing crypto runtime，在高核主机上线性扩展，同时限制混合负载中的 TLS CPU 份额并避免固定热点核。正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标，内存以 current/peak/每连接成本报告，只有实际生产预算才设上限。达到每 base shard 64 条活跃 TLS 连接时，额外 TLS accept shard 才弹性启动。
...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder，不再走 HTTP/1 自动探测；TLS accept、handshake 与 H2 connection 全程留在 `ceil(cpuset cores / 2)`、nice +3 的独立 crypto runtime，避免中途跨 runtime wake。mixed H2 budget 为每 cpuset CPU 3,800 ops/s；`SO_REUSEPORT` 分散连接。小静态热对象以 immutable `Bytes` 应答并使用 stale-while-revalidate；Range 精确返回 `206`/`416`。大文件 Linux fast lane 复用缓存 fd 与 `sendfile`，chunk 为 `small=2 MiB`、`balanced=16 MiB`、`bulk=16 MiB`。默认 `balanced` 由固定 HTTP connection owner 在 Tokio writable readiness 上 drain，三相 cooperative yield 避免 16/32 条连接同步形成 convoy；`bulk` 显式使用原生 epoll sendfile reactor。`sendfile(2)` 使用每响应独立 offset，reactor queue 满或 handoff 失败时回退 Tokio readiness；`small` 保留 125µs 公平 pacing。HTTP shard 保持 nice 0，独立 UDP、TLS、realtime runtime/reactor 均为 nice +3，使混合负载共享空闲 CPU但不互相饿死。所有 profile 变更都必须重跑完整 mixed gate，不能只报大文件单项。
...
- 60 秒 validation deadline 会下传到每个 client wave，并由 GNU `timeout` 按剩余预算硬终止；超时同时停止容器内发生器并失败，不再只在矩阵结束后检查。默认统一启动 lead 为 250 ms，UDP/QCP 尾部等待 1 秒，3 秒测量窗口不缩短。

...
- Linux `runtime.performance.enabled=true` 下，plain HTTP 与 balanced UDP 复用每核一个 nice 0 shard；每个 plain accept shard 建立 4 个 shard-local `FuturesUnordered` owner group，只轮询已就绪连接，避免每连接 Tokio task 在高并发下放大全局 scheduler 成本，同时保持 fd 与 reactor 绑核。连接 round-robin 分入 4 组，每个 ready connection 最多连续完成 8 个 fast-lane 响应便重新排队，在吞吐摊销与队头延迟之间设硬边界；非 owner fallback 仍使用 8/32 响应批次。UDP 热关联每 8 包让出一次，realtime reactor 每 4 CPU 一个 nice +5 owner并在连续 8 个 ready batch 后让出。TLS 使用 `ceil(cpuset cores / 8)`、nice 0 的独立 runtime。配置 hot reload 通过 ArcSwap 原子发布。

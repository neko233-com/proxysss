...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 性能模式从 cgroup effective cpuset（而非可能已被 pin 的当前线程）发现完整 CPU 集，并据此建立全核 `SO_REUSEPORT` listener fanout 与 CPU 自适应 native reactor owners。balanced 原生 WebSocket/TCP epoll relay 每 4 CPU 一个 nice +5 owner，持续 8 个 ready batch 后让出；balanced UDP 与 plain HTTP 复用 per-core shard，仅当 8 个热关联数据报在 8ms 内形成持续 burst 时 cooperative yield；默认 small 的独立 UDP/QCP runtime 每 2 核一个 owner。默认 small 的 TLS 使用 `ceil(cpuset cores / 4)`，balanced 使用 `ceil(cpuset cores / 8)`；两者都是 nice 0 的单/少 owner crypto runtime。配置通过 ArcSwap 原子发布；balanced 大文件留在 HTTP owner，bulk 才启用 sendfile reactor。
...
- Linux `runtime.performance.enabled=true` 下，从 cgroup effective cpuset 发现完整 CPU 集；plain HTTP 与 balanced UDP 复用每核一个 nice 0 shard，balanced UDP 只在 8 包/8ms 的持续 burst 下让出，默认 small UDP/QCP 使用每 2 核一个 owner 的独立 runtime；realtime reactor 每 4 CPU 一个 nice +5 owner并在连续 8 个 ready batch 后让出。默认 small TLS 使用 `ceil(cpuset cores / 4)`，balanced 使用 `ceil(cpuset cores / 8)`，均为 nice 0 独立 runtime。plain fast lane 在少于 300 活跃连接时每 8 响应让出，300 以上每 32 响应让出。配置 hot reload 通过 ArcSwap 原子发布。
- plain HTTP 静态 fast lane 在首个请求完成路由、header 与缓存校验后，把完整原始请求头作为同一 keep-alive 连接的下一次读取边界；完全匹配时不再重复扫描 `CRLFCRLF` 和二次逐字节比较。分片读取、pipeline leftover、请求变化与 1 秒缓存重验证仍走原语义。
...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder；TLS 全程留在 profile 对应的 bounded、nice 0 crypto runtime（small 每 4 核一个，balanced 每 8 核一个）。H2 小静态热对象使用 immutable `Bytes` 与 lock-free 配置快照。balanced 大文件在固定 HTTP owner 上 drain `sendfile`，bulk 启用全核 reactor。release 使用 fat LTO 与单 codegen unit。所有 profile 变更都必须重跑完整 mixed gate。

...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载；worker 数以 `sched_getaffinity` 的 cpuset 为准，不设固定小上限。plain `ws://`、游戏小帧 TCP 与 WSS 都使用多核 Tokio relay，避免单 reactor 唤醒瓶颈；正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标，内存以 current/peak/每连接成本报告，只有实际生产预算才设上限。TLS 低密度连接复用 HTTP shard；达到每 base shard 64 条活跃 TLS 连接时，额外 TLS shard 才弹性启动。WSS 始终保留 rustls/AWS-LC 语义。
...
- plain WebSocket 在 HTTP shard 完成握手后继续使用多核 Tokio relay；WSS 使用同样的有界 relay 加 rustls/AWS-LC。空闲长连接不会预占每方向固定大 buffer。4c 的生产参考包络默认验证 20k idle WSS，活跃消息规模验证到 4096，并分别报告握手与消息 p50/p95/p99、内存 current/peak 与每连接成本；它不是固定 RAM 准入门槛。

...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 且 `runtime.performance.enabled=true` 时，listener 按 `sched_getaffinity` 检测到的可用 CPU 核建立 `SO_REUSEPORT` accept worker。默认 `balanced` 把 plain HTTP、plain WebSocket、TCP accept/relay 与 UDP recv/association 放在同一组 per-core Tokio I/O shard；worker index 决定 connection owner，连接不跨 runtime，但 shard thread 使用 cpuset 内 soft affinity，vCPU 暂停或负载不均时允许 Linux CFS 迁移 owner。`small`/`bulk` profile 仍硬 pin。这样既避免再创建常驻 realtime `epoll` 线程及跨 runtime wake，也不会把整个 connection 永久困在偶发停顿的 vCPU 上。每条 TCP/WS connection 在同一 owner 内维护 backpressure 与 half-close，普通 realtime 走 async bounded relay，明确的 bulk/file profile 才使用 Linux `splice`。TLS 保留 `ceil(cpuset cores / 2)`、nice +7 的独立有界 crypto runtime，防止 rustls/AWS-LC 抢占 plain HTTP/UDP/realtime 的全部 CPU，又不会让 HTTPS 长期饥饿。
...
4. TCP 默认关闭 Nagle（`nodelay: true`）并应用 `connect_timeout_ms`。Linux 性能模式下，游戏、MQTT/tool 与通用实时流留在 CPU 自适应的 per-core Tokio I/O shard，直接使用 async backpressure；明确的 bulk/file 协议才使用带 `SPLICE_F_MORE` 的 Linux `splice` 零拷贝。
...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载；worker 数以 `sched_getaffinity` 的 cpuset 为准，不设固定小上限。默认 `balanced` 让 plain HTTP、plain `ws://`、TCP accept/relay 与 UDP recv/association 共用 per-core Tokio runtime shard，worker index 决定 connection owner；连接不跨 runtime，shard thread 则在 cpuset 内使用 soft affinity，让 CFS 在 vCPU 暂停或 runnable 不均时迁移 owner。`small`/`bulk` 仍硬 pin。connection-local data runtime 以 31 次 task poll 摊销一次 global injection queue 检查、以 8 次 task poll 摊销一次 I/O/timer driver 检查，在降低 scheduler bookkeeping 的同时保护 HTTPS/realtime p95/p99。WSS 因 rustls 语义保留 Tokio 有界 relay。TLS 使用 `ceil(cpuset cores / 2)`、nice +7 的独立 work-stealing crypto runtime，在高核主机上线性扩展，同时限制混合负载中的 TLS CPU 份额并避免固定热点核。正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标，内存以 current/peak/每连接成本报告，只有实际生产预算才设上限。达到每 base shard 64 条活跃 TLS 连接时，额外 TLS accept shard 才弹性启动。

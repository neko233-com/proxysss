...
1. 在配置的 `tcp.listeners[]` / `udp.listeners[]` bind 接收连接或数据报。Linux 且 `runtime.performance.enabled=true` 时，listener 按 `sched_getaffinity` 检测到的可用 CPU 核建立 `SO_REUSEPORT` accept worker；游戏小帧、通用 TCP 与 plain WebSocket 交给按 cpuset 线性扩展的原生 `epoll` relay reactor：多核 cpuset 至少双 owner，之后 `small` 每 2 个可用 CPU 一个 owner，默认混合负载 `balanced` 与 `bulk` 每 4 个 CPU 一个 owner，为并发 HTTP/TLS/UDP/file 保留 scheduler 预算。worker 数少于 cpuset 核数时使用 soft CPU ownership，由 Linux 在 runnable imbalance 时迁移；一核一 owner 时反向硬 pin。双核不会再把全部长连接串行放进单个 owner。owner 空闲时阻塞在 epoll，balanced 用 nice +5 在同核 HTTP/TLS 可运行时有界让出预算。每条连接只由一个 reactor owner 维护 backpressure 与 half-close，避免每帧跨 Tokio 调度；`EPOLLHUP`/`EPOLLRDHUP` 与 `EPOLLIN` 同时出现时先 drain 尾部数据再传播 half-close，不能丢弃游戏/WebSocket 最后一帧。每 worker 不超过 4 对低密度连接时只做极短 reply spin，中高密度完全依赖 `epoll` batch；不可 handoff 或非 Linux 时回退到有界 Tokio relay，bulk 流保留 `splice` 路径。默认 `balanced` 把 plain HTTP、TCP accept 与 UDP recv/association 固定到同一组 per-core shard：同一 `SO_REUSEPORT` worker 的 socket 始终留在同核 Tokio reactor，避免为 UDP 再创建竞争线程、跨 runtime wake 与 cache migration。TLS 保留独立的有界 crypto runtime，防止 rustls/AWS-LC 计算挤占 small HTTP/UDP shard；原生 stream/sendfile reactor 仍独立承担不适合 async task 状态机的大吞吐 relay。
...
- Linux `runtime.performance.enabled=true` 下，plain HTTP、TLS HTTP、TCP stream、UDP listener 都由 `SO_REUSEPORT` 多 worker 承载；worker 数以 `sched_getaffinity` 的 cpuset 为准，不设固定小上限。plain `ws://` 与小帧 TCP 进入原生 `epoll` relay：多核至少双 owner，之后 `small` 每 2 个可用 CPU 一个 owner，默认混合负载 `balanced` 与 `bulk` 每 4 个 CPU 一个 owner；所有 owner 都随 cpuset 线性增长，空闲时阻塞。WSS 因 rustls 语义保留 Tokio 有界 relay。默认 `balanced` 让 plain HTTP、TCP accept、UDP recv/association 共用 per-core runtime shard，worker index 决定 CPU owner；connection-local data runtime 以 31 次 task poll 摊销一次 global injection queue 检查、每 16 次 task poll 驱动一次 I/O/timer，保留 mixed 小包吞吐；更短的 4 次间隔会让 data-shard 饱和吞吐下降，不能用低负载尾延迟单项替代 mixed gate。TLS 使用多核至少双 worker、之后 `ceil(cpuset cores / 4)`、nice +3 的独立 work-stealing crypto runtime；它在高核主机上线性扩展，同时限制混合负载中的 TLS CPU 份额并避免单 worker 队头阻塞。正式 4c 包络以 4096 active WSS 和 20k idle WSS 为默认可验证目标，内存以 current/peak/每连接成本报告，只有实际生产预算才设上限。达到每 base shard 64 条活跃 TLS 连接时，额外 TLS accept shard 才弹性启动。
...
- TCP stream 有独立 relay profile。Linux 性能模式下，游戏小帧与通用 1 KiB realtime TCP 默认使用 CPU 自适应分片的原生 `epoll` relay，每条连接由单一 reactor owner 维护显式 backpressure 和 half-close；多核 cpuset 至少双 owner，之后 `small` 每 2 个 cpuset CPU 一个 owner，`balanced` 与 `bulk` 每 4 个 CPU 一个 owner，三者都随高核 cpuset 线性增长而不设固定上限。reactor worker 数少于 cpuset 核数时使用 soft CPU ownership，让 Linux 把长寿命 owner 留在当前 CPU，只有出现 runnable imbalance 才迁移；一核一 owner 时才反向硬 pin。双核双 owner 会分别 pin 到两个核，消除所有长连接共用单线程的队头阻塞。`small` realtime 优先 profile 保持 Linux nice 0；混合 `balanced` 与 `bulk` owner 使用 nice +5，让 CFS 在同核 HTTP/file task 可运行时有界让出 CPU，但不会像旧 +12 权重那样在 gateway 尚有空闲预算时仍把 WebSocket/game/TCP wake 延迟到 nginx 之后；空闲时连续处理 epoll batch，不用逐 batch `sched_yield` 制造与 CPU 速度无关的吞吐平台。handoff 不可用或非 Linux 时使用多核 Tokio 双向 relay 与有界 `ByteBufferPool`，bulk/file/backup profile 才使用 `splice(socket -> pipe -> socket)`。
- 双 lane floor 只改变并行度，不允许翻倍抢占 mixed CPU：当自然的 `ceil(cores / divisor)` 只有 1 而多核 floor 启动 2 个 worker 时，balanced/bulk realtime 从基准 nice +5 调整为每 worker nice +8，TLS 从基准 nice +3（bulk 为 +5）调整为每 worker +6（bulk 为 +8）。两条 lane 的 CFS 总权重约等于原单 lane；达到自然双 worker 的核心数后恢复基准 nice。这样消除单 owner 队头阻塞，同时不牺牲 static、reverse、SSE、UDP/QCP sibling。
...
- HTTP/2 在 ALPN 已确认后直接使用 Hyper H2 server builder，不再走 HTTP/1 自动探测；TLS accept、handshake 与 H2 connection 全程留在独立 crypto runtime 的同一 connection owner，避免中途跨 runtime 搬运和 wake。mixed H2 budget 为每 cpuset CPU 3,600 ops/s，利用旧 2,000 上限留下的 gateway 空闲预算，同时继续给 plain HTTP/UDP/realtime 留出确定份额。`SO_REUSEPORT` 把不同连接分散到 CPU 自适应 shard。小静态文件进入有界内存缓存，H2 热对象以 immutable `Bytes` 直接应答，并使用 stale-while-revalidate。Range 精确返回 `206`/`416`；大文件 Linux fast lane 使用缓存 fd + `sendfile`，单次 chunk 随 traffic profile 自适应为 `small=2 MiB`、`balanced=16 MiB`、`bulk=16 MiB`。默认 `balanced` 让 accept shard 的固定 connection owner 直接在 Tokio writable readiness 上 drain partial writes，直到 chunk 预算、EOF 或 `EAGAIN`，避免低密度 connection/per-response runtime migration、fd duplication、eventfd、epoll add/del 与 oneshot 往返；socket 在 `EAGAIN` 时自然让出，完整 sendfile 响应后 cooperative yield 一次。为避免高并发 bulk connection 连续占住单线程 owner，同时保留大文件吞吐，每条 connection 用本地 sequence 让每 3 个响应中的 2 个在 8 MiB 中点额外 yield、另 1 个保持完整 16 MiB drain；sequence 的初相位由 remote source port 分散到三相，避免 16/32 条连接同时在 8 MiB 形成 scheduler convoy。`balanced` 在 active large responses 达到 `1 × cpuset cores` 时动态把后续 duplicated socket/file fd 交给有界、`ceil(cpuset cores / 4)` 分片、round-robin 的原生 epoll sendfile reactor；计数发生在进入当前响应前，因此每核 2 条持续 keep-alive large connection 也会在稳态全部迁出 HTTP owner，不会卡在临界值。sendfile owner 固定在 cpuset 前部；sparse realtime relay 使用 soft ownership，一核一 owner 才固定到 cpuset 后部。HTTP/TCP accept/UDP 共用的 per-core shard保持 nice 0，TLS crypto runtime 使用 nice +3，sendfile owner 使用 nice +6、realtime owner 使用 nice +5，只对真正独立的 runtime/reactor 做 CFS 加权。低于基线门槛继续直送，达到门槛后逐步 offload；阈值与 worker 数都随高核主机线性增长，明确选择 `bulk` 则始终使用全核 nice 0 reactor。密度只在响应边界使用一个 relaxed active counter，不在 chunk 热循环加锁。`sendfile(2)` 使用每响应独立的显式 offset，queue 满或 handoff 失败时从当前 offset 回退 Tokio readiness。默认 `small` 仍做 125µs 公平 pacing。要求同一轮把 small/realtime 与 bulk 都设为严格 gate 时，可显式使用 `TRAFFIC_PROFILE=balanced`，但必须重新通过全部混合场景，不能只报大文件单项。

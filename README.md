...
- 主动 + 被动健康检查：HTTP/TCP 探测、失败阈值隔离、成功恢复、维护态持久化
- 重试策略：可配置最大重试次数
...
- 扩展服务：内建 `services.static_sites` 静态文件服务、`services.ftp` 原生控制通道代理 + 主动/被动数据通道、内建 `services.webdav` 运行时
...
proxysss -c ./myconfig.yaml check-config
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/neko233-com/proxysss/main/scripts/install.sh | bash -s -- v0.3.7
```
...
& ([ScriptBlock]::Create((irm https://raw.githubusercontent.com/neko233-com/proxysss/main/scripts/install.ps1))) -Action update -Version v0.3.7
...
proxysss update --version v0.3.7
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\install.ps1 -Action upgrade -Version v0.3.7
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\install.ps1 -Action update -Version v0.3.7 -DryRun
```
...
```bash
bash ./scripts/install.sh v0.3.7
...
- 自动签发/续期已走内置 managed ACME（HTTP-01 / TLS-ALPN-01），`acme_external` 仅作为兼容路径保留。
...
`http.tls.auto_https` 是 proxysss 风格的高层入口；`services.domain_routes[*].ssl.type` / `is_auto_ssl` 则是更贴近“域名即配置核心”的入口。当前默认会走内置 managed ACME 签发/续期；`self_signed` 只适合开发或内网。`services.access_control.http` 可直接补上公网 IP 黑名单 / 白名单，`services.rate_limit.http` 可直接补上内建请求限流。
...
- GET /v1/stats
- GET /v1/upstreams
- POST /v1/upstreams/disable
- POST /v1/upstreams/enable
- GET /v1/config
- POST /v1/reload
- GET /v1/plugins
...
| FTP 入口 | ⬜ | 部分完成 | 已支持原生控制通道代理、被动/主动数据通道改写，仍在补更深的命令级策略 |
...
| 自动 HTTPS 完全内置零外部依赖 | ✅ | 已完成 | 内置 managed ACME 支持 HTTP-01 / TLS-ALPN-01 |
| 访问日志 / 错误日志 | ✅ | 已完成 | `logs/access.log` / `logs/error.log` |
| 热重载 | ✅ | 已完成 | 配置、include、脚本、插件都进 fingerprint |
| 压缩 | ⬜ | 部分完成 | 已支持 zstd + gzip + brotli；更细的策略层还在继续补齐 |
| IP 黑名单 / 白名单 | ✅ | 已完成 | `services.access_control.http.allow/deny` 支持 IP / CIDR |
| 缓存 / proxy cache | ⬜ | 部分完成 | 已支持共享缓存区、磁盘缓存、PURGE；更高级 revalidation/variant 仍在继续补齐 |
| 限流 | ⬜ | 部分完成 | 已支持共享区风格请求限流和连接数限制，更多 shaping 策略仍在继续补齐 |
...
| 场景 | 当前状态 | 说明 |
| --- | --- | --- |
| 普通反向代理 / 多 upstream | 已支持 | YAML `services.reverse_proxy.routes` + 负载均衡 + 重试 + 主动/被动健康检查 + 维护态 |

...
The one-minute limit is enforced inside every client wave with the remaining budget; expiry terminates the in-container load generators and fails the run. The synchronized start lead is 100 ms and UDP/QCP tail timeout is 1 second, keeping the matrix inside the cap without shortening its 3-second measured windows.

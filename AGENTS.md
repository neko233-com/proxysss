...
- The Ubuntu 24 x86_64 strict workflow starts static-large at two equal flows per detected CPU for both gateways, then scales every workload together with `load_scale`; one bulk flow per `SO_REUSEPORT` shard is a sparse hash-placement diagnostic, not the saturated release gate.

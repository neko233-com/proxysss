...
- Linux performance validation must prioritize mixed multi-proxy load over isolated single-scenario wins: CDN/static delivery, HTTP reverse proxy, New API/SSE, game/TCP streams, generic TCP streams, UDP streams, and KCP-style UDP must run together per gateway. Single-scenario runs are diagnostic only and must not be used to justify release success by cherry-picking.
...
For Ubuntu/Linux performance validation, run `proxysss tune linux --apply` first, then run `scripts/benchmark-all-scenarios.sh` on the Linux host or Ubuntu 24 benchmark container. The default release gate is mixed-load (`MIXED_MATRIX=1`) and Linux-only: nginx runs first, then proxysss runs the same concurrent scenario wave across static-small, static-large, CDN hot-update static, HTTPS static, HTTP reverse proxy, New API/SSE, WebSocket long connections, game long-connection TCP, TCP stream, UDP stream, and KCP-style UDP. The default critical gate uses fair proxysss/nginx ratios of `CRITICAL_RATIO=0.97` for WebSocket, game TCP, generic TCP, UDP, and KCP-style UDP plus `AGGREGATE_RATIO=0.97`; static/reverse/SSE must remain above the soft floor and error tolerance. Single-scenario runs are for root-cause analysis only, and strict `>1.0` gates must be explicitly requested.

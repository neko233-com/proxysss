...
- The strict Docker matrix measures steady gateway data-path latency: before the shared absolute start, both candidates preconnect plain HTTP/1 pools at the declared scenario concurrency and HTTPS/H2 preconnects its multiplexed session, then execute and drain two real GETs with a short gap so static/reverse data caches and stale-while-revalidate work finish before measurement. Client connection/resource warm-up and the default 1000 ms future-start lead are excluded from active measurement; cold-connect and cold-cache capacity are separate diagnostics and must not contaminate one-second steady p99.

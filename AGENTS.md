...
- Balanced Linux connection-local runtimes amortize global-queue checks over 31 task polls but drive I/O/timers every 8 polls to protect mixed HTTPS/realtime tails. Balanced TLS uses `ceil(cpuset cores / 2)` workers at nice +6. The TLS static fast lane sends the response header and immutable cached body through rustls' vectored writer, then completes any remainder with async backpressure; do not rebuild a contiguous response buffer and copy the cached body on every request.

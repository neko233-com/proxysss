...
- Domain-only automatic WSS uses free built-in managed ACME with `http.tls.auto_https.domains`; its production default is zero-external-tool TLS-ALPN-01 (DNS plus public 443) and ECDSA P-256 certificate keys. `http.tls.acme.key_algorithm: rsa2048` is the explicit legacy-client compatibility option, while explicit `challenge: http01` remains supported for existing port-80 deployments.
...
- The default `small` traffic profile keeps HTTP/1.1, HTTP/2/TLS, WebSocket, TCP, UDP, and transparent QCP on one CPU-sized Tokio data runtime with one worker per allowed core and work stealing across those per-core shards. `balanced` keeps HTTP/1.1 on that runtime, gives TLS/H2 one CPU-adaptive bounded runtime at nice 5, and gives both UDP/QCP and post-connect policy-free realtime TCP/plain WebSocket CPU-adaptive pools sized at `ceil(cores/2)` with nice 3. Established TCP/WebSocket pairs use the native epoll relay; UDP keeps Tokio socket ownership. This removes long-lived relay futures from the HTTP ready queue while nice 3 protects HTTP saturation. The rejected `ceil(cores/4)` stream pool at nice 0 fell to 0.85-0.98x nginx and worsened realtime p95/p99, so do not restore that shape. Putting continuously runnable H2 back onto the shared balanced runtime caused 16-21 ms cross-protocol tails and up to a 0.225x HTTPS saturation ratio. `bulk` may use dedicated transfer owners. Tokio's LIFO slot remains enabled. Plain static keeps an explicit 256-response low-density / 128-response high-density fairness boundary; raw reverse/SSE rely on their bounded upstream/downstream readiness points. Balanced dedicated UDP relies on Tokio socket scheduling; shared-small UDP retains its 4-packet fairness boundary. Static file revalidation uses a 2-second freshness window and one nice-10 control runtime. Freshness timestamp and in-flight claim are shared atomics, so stale readers never take a DashMap write lock; unchanged metadata refreshes shared freshness without rebuilding the H2 snapshot. Small-profile WebSocket relays use the normal 64-poll readiness-bounded turn. The global injection queue remains at 31 polls and the I/O driver at 16 polls. HTTP/2 uses fixed large windows plus a warm-up-built immutable ArcSwap/FxHashMap response snapshot. Each H2 connection drives stream futures through one local `FuturesUnordered` executor instead of one Tokio spawn/global-queue operation per request and yields after every completed ready stream; a dedicated 32-stream batch was rejected because its CFS consumption collapsed 4x CDN/static/reverse throughput even though H2 itself was fast. Do not put same-path H2 requests behind a DashMap shard or populate the snapshot on first live traffic. Per-request metrics use shard-local padded counters. Small-profile sendfile uses 8 MiB chunks only while active transfers do not exceed detected data-plane cores, then automatically returns to the mixed-safe 2 MiB slice.
...
- One-second equal-offered-load plans must remain mathematically executable: HTTP/SSE workers use centered uniform phase offsets and a rounded aggregate schedule; synchronized WebSocket/TCP/UDP workers use only whole per-connection ticks strictly before the deadline. A worker waiting beyond the measurement deadline exits immediately instead of extending process grace.
- Persistent Docker matrices pause the inactive gateway. After each unpause, both nginx and proxysss must receive the same default 250 ms settle window outside active measurement before the future start time is scheduled; otherwise cold CPU caches and overdue timer/I/O bookkeeping can create candidate-wide false p99 spikes.
- The default 20-second feedback matrix spends 6 seconds on one full saturation pass and 12 seconds on two equal-load repetitions with reversed candidate order, for 18 seconds active measurement. Latency reports use the two-sample median; do not revert to a single p99 sample when the budget can cover balanced repetitions.
- The strict Docker matrix measures steady gateway data-path latency: before the shared absolute start, both candidates preconnect plain HTTP/1 pools at the declared scenario concurrency and HTTPS/H2 preconnects its multiplexed session, execute and drain two real GET/SSE resource requests, and complete one WebSocket/TCP/UDP/QCP echo on every declared connection. This finishes static/reverse cache work, stale-while-revalidate, stream relay activation, and UDP association creation before measurement. Client connection/resource/session warm-up and the default 2000 ms future-start lead are excluded from active measurement; cold-connect, cold-cache, and cold-association capacity are separate diagnostics and must not contaminate one-second steady p99.
...
- Config load and hot reload must trigger a fast self-optimization warm-up before listeners serve traffic: preload hot static files into the bounded fast-lane cache, prebuild reusable HTTP/1 keep-alive bytes for eligible small objects, and pre-dial a small bounded pair of reverse-proxy/AI-proxy upstream keepalive sockets per deduplicated pool. Do not bulk pre-dial sockets on the startup runtime and then use them as long-lived data-shard lanes; Tokio socket registration remains tied to the creating reactor and cross-runtime ownership regresses latency. Numeric upstream IPs must be parsed once into `SocketAddr` instead of repeating string address resolution on every cold connect. Warm-up reruns after every reload, and `/healthz` exposes a `warm` readiness flag. Benchmarks should start only after warm-up.
...
Default GitHub Actions CI is pure packaging: six release bundles are built and uploaded. GitHub Actions must not contain, schedule, or manually dispatch tests, smoke benchmarks, or performance benchmarks. For scenario-surface Docker validation, run `scripts/verify-docker-scenarios.sh` on Linux/macOS or `scripts/verify-docker-scenarios.ps1` on Windows; it checks `examples/all-scenarios.example.yaml`, static Range downloads, service discovery config, capability output, and nginx-parity declarations in an Ubuntu 24 container. `REUSE_VERIFY_IMAGE=1` may reuse its local image during registry outages, but the script must still probe Ubuntu 24.04 x86_64 before testing. For local Ubuntu/x86 performance validation, run `scripts/benchmark-ubuntu24-amd64-docker.sh`; Windows Docker Desktop is supported from Git Bash through its local `npipe://` endpoint, while Linux/macOS use a local Unix socket. On an arm64 Docker host the script requires Zig + cargo-zigbuild + the Rust x86_64-unknown-linux-gnu target, cross-builds at native host speed, and executes the resulting ELF in Ubuntu 24 amd64 before measurement; it hard-checks Ubuntu 24.04 x86_64, records native versus emulated amd64, and drives gateway/backend/client containers on disjoint cpusets so faster closed-loop traffic cannot steal CPU from the gateway under test. Each wave uses one client container with 11 independent benchmark processes; each scale keeps one backend and both gateways warm while pausing the inactive gateway, cutting container startup out of the 1-second samples. The strict 1x/2x/4x mixed and equal-offered-load matrices include static-small, static-large, CDN hot-update static, HTTPS static, HTTP reverse proxy, generic SSE, WebSocket long connections, game long-connection TCP, TCP stream, UDP stream, and transparent QCP; every row requires zero errors and strict superiority. The default 20-second active-measurement gate uses a 1-second window with one saturation sample and two reversed-order equal-load samples per gateway/scale; latency reports use their median. Build, image setup, container preparation, warm-up, process startup, result collection, and report parsing are excluded from `validation_elapsed_secs`, while `validation_wall_elapsed_secs` records matrix orchestration time separately. Equal-load defaults to 25% of the slower gateway saturation rate, leaving latency headroom while all 11 isolated generators run; delayed client timer slots are caught up under bounded per-worker concurrency instead of being silently dropped. `proxysss bench` and benchmark fixture `demo` processes must build their child Tokio runtime from `TOKIO_WORKER_THREADS`; the single-thread supervisory runtime must never silently override the declared client/backend resource budget. Saturation clients retain the full client cpuset; fixed-rate equal-load uses two bounded Tokio I/O workers for HTTP/SSE, one for realtime WebSocket/TCP/UDP/QCP, and two for static-large. Both candidates receive the same client resources and must complete at least 98% of target. nginx remains mainline 1.31.2 built with `-O3 -fno-plt`, the same gateway cpuset, and equivalent protocol configuration. The older shared-role diagnostic is `scripts/benchmark-all-scenarios.sh` with `MIXED_MATRIX=1`; its default fair release floor remains `CRITICAL_RATIO=0.97` for WebSocket, game TCP, generic TCP, and nginx-comparable UDP plus `AGGREGATE_RATIO=0.97`. New API provider routes and KCP special encapsulation are excluded from the default head-to-head matrix; QCP evidence is transparent UDP edge forwarding and must not claim frame termination. Single-scenario runs are diagnostic only and must not justify release success by cherry-picking. v1.3.5 UDP-only Docker validation for `udp-stream` passed at `4.045x` (`127742.75` vs `31577.33` ops/s, 0 errors).
The 20-second limit is an active client measurement budget accumulated from every gateway/phase/scale window. Before each wave, the runner must reserve its full measurement duration; each in-container wave also has a small process grace timeout that terminates stuck generators and fails the run. Build/setup/warm-up/process startup/result copy/report parsing stay outside the active budget and are reported separately as wall time. `REUSE_BENCH_IMAGE=1` may reuse a local controller image when registry metadata is unavailable, but the wrapper must still probe it as Ubuntu 24.04 amd64 before measurement. Benchmark helpers in that flow are Go-based (`scripts/benchmark-helper.go`); do not swap in Python fixtures or parsers for official runs.

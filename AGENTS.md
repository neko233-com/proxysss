...
- Balanced Linux connection-local runtimes amortize global-queue checks over 31 task polls but drive I/O/timers every 8 polls to protect mixed HTTPS/realtime tails. The TLS static fast lane sends the response header and immutable cached body through rustls' vectored writer, then completes any remainder with async backpressure; do not rebuild a contiguous response buffer and copy the cached body on every request.
- Balanced per-core Tokio shards keep each connection on one single-thread owner but use soft cpuset affinity, allowing Linux CFS to migrate the owner thread when a vCPU is stalled or unevenly loaded. `small` and `bulk` keep hard CPU pinning; do not reintroduce hard pinning for `balanced` without mixed p50/p95/p99 evidence.
- Preloaded HTTP/2 small-static routes use one DashMap route lookup on the first stream plus a connection-local `OnceLock` hit for later exact-path streams, then read an `ArcSwap` immutable payload. The payload shares the existing cached `Bytes`, keeps lock-free hot reads, and is replaced by stale-while-revalidate when the file changes; do not restore a second per-request static-file map lookup or a global hot-path lock.
...
- Static sites should participate in config-load optimization: after loading config, eligible hot index/top-level static files are preloaded into the bounded cache or have sendfile descriptors prepared according to `runtime.performance.traffic_profile`. `small` preloads bodies through 1 MiB; `balanced` preloads cacheable bodies below the 32 MiB streaming boundary; both remain inside the shared 256 MiB/256-entry limits, while `bulk` prepares sendfile state instead of body copies.

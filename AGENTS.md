...
- Balanced Linux connection-local runtimes amortize global-queue checks over 31 task polls but drive I/O/timers every 8 polls to protect mixed HTTPS/realtime tails. The TLS static fast lane sends the response header and immutable cached body through rustls' vectored writer, then completes any remainder with async backpressure; repeated exact request heads reuse a per-connection parsed response cache for at most the static revalidation interval. Do not rebuild a contiguous response buffer, copy the cached body per request, or bypass the freshness deadline.

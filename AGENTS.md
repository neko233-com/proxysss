...
- Config load and hot reload must trigger a fast self-optimization warm-up before listeners serve traffic: preload hot static files into the bounded fast-lane cache, prebuild reusable HTTP/1 keep-alive bytes for eligible small objects, and pre-dial reverse-proxy/AI-proxy upstream keepalive pools so the first live request never pays allocation/serialization or a cold connect. Linux pre-dial is parallel, CPU-adaptive, deduplicated per pool, and bounded globally/per upstream. Warm-up reruns after every reload, and `/healthz` exposes a `warm` readiness flag. Benchmarks should start only after warm-up.

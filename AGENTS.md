...
- Balanced Linux plain/UDP connection-local runtimes amortize global-queue checks over 31 task polls; the independent TLS/H2 runtime checks its injection queue every 8 polls so multiplexed stream tasks are not delayed. All drive I/O/timers every 8 polls to protect mixed HTTPS/realtime tails. The TLS static fast lane sends the response header and immutable cached body through rustls' vectored writer, then completes any remainder with async backpressure; do not rebuild a contiguous response buffer and copy the cached body on every request.
- Preloaded HTTP/2 small-static routes use one DashMap route lookup plus an `ArcSwap` immutable payload. The payload shares the existing cached `Bytes`, keeps lock-free hot reads, and is replaced by stale-while-revalidate when the file changes; do not restore a second per-request static-file map lookup.

...
- The default `small` traffic profile keeps HTTP/1.1, HTTP/2/TLS, WebSocket, TCP, UDP, and transparent QCP on one CPU-sized Tokio data runtime with one worker per allowed core and work stealing across those per-core shards; Tokio's non-stealable LIFO slot stays disabled so echo-style wake chains enter stealable queues instead of creating mixed-load p99 stalls. Low-density HTTP/TLS/WebSocket lanes must rely on socket readiness and the Tokio cooperative budget instead of sub-p99 self-wakes: explicit HTTP yields are amortized over 256 responses until connections exceed 256 per data worker, then use the 32-response saturated fairness boundary. Do not reintroduce separate always-runnable TLS/UDP runtime pools or isolated one-thread runtimes that preserve SO_REUSEPORT hash skew under mixed load. Per-request metrics must use shard-local padded counters instead of one cross-core cache line. Small-profile sendfile keeps a 2 MiB cooperative slice; a 16 MiB ceiling was rejected because it starved mixed HTTP siblings on shared shards. `balanced` may share UDP while retaining its bounded TLS runtime, and `bulk` may use dedicated transfer owners.

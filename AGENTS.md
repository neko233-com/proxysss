...
- The direct Ubuntu 24 x86_64 Docker strict matrix starts static-large at two equal flows per detected CPU for both gateways, then scales every workload together at 1x/2x/4x; one bulk flow per `SO_REUSEPORT` shard is a sparse hash-placement diagnostic, not the saturated release gate. GitHub Actions must never run performance benchmarks; it is packaging-only.

...
- **Release tags require a matching changelog section on the same commit.** Before pushing `vX.Y.Z`, ensure `CHANGELOG.md` contains `## vX.Y.Z` and `Cargo.toml` `version` matches. The default CI is packaging-only; `release.yml` still validates the changelog section during publish.
...
Default GitHub Actions CI is pure packaging: six release bundles are built and uploaded, and tests/smoke/performance benchmark jobs are not run automatically. For scenario-surface Docker validation, run `scripts/verify-docker-scenarios.sh` on Linux/macOS or `scripts/verify-docker-scenarios.ps1` on Windows; it checks `examples/all-scenarios.example.yaml`, static Range downloads, service discovery config, capability output, and nginx-parity declarations in an Ubuntu 24 container. For Ubuntu/Linux performance validation, run `proxysss tune linux --apply` first, then run `scripts/benchmark-all-scenarios.sh` on the Linux host or Ubuntu 24 benchmark container. The default manual release gate is mixed-load (`MIXED_MATRIX=1`) and Linux-only: nginx runs first, then proxysss runs the same concurrent scenario wave across static-small, static-large, CDN hot-update static, HTTPS static, HTTP reverse proxy, generic SSE, WebSocket long connections, game long-connection TCP, TCP stream, and UDP stream. The default critical gate uses fair proxysss/nginx ratios of `CRITICAL_RATIO=0.97` for WebSocket, game TCP, generic TCP, and nginx-comparable UDP plus `AGGREGATE_RATIO=0.97`; static/reverse/generic SSE must remain above the soft floor and error tolerance. New API provider routes and KCP/QCP special UDP encapsulations are excluded from the current performance benchmark matrix. Single-scenario runs are for root-cause analysis only, and strict `>1.0` gates must be explicitly requested. v1.3.5 UDP-only Docker validation for `udp-stream` passed at `4.045x` (`127742.75` vs `31577.33` ops/s, 0 errors).

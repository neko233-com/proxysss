...
- Balanced Linux connection-local runtimes amortize global-queue checks over 31 task polls, drive I/O/timers every 8 polls, and cap each HTTP/TCP/UDP I/O driver tick at 128 readiness events while TLS/H2 keeps 256 to protect mixed tails without excessive crypto-runtime polling. The TLS static fast lane sends the response header and immutable cached body through rustls' vectored writer, then completes any remainder with async backpressure; do not rebuild a contiguous response buffer and copy the cached body on every request.

...
- The default `small` traffic profile keeps HTTP/1.1, HTTP/2/TLS, WebSocket, TCP, UDP, and transparent QCP on one CPU-sized Tokio data runtime with one worker per allowed core and work stealing across those per-core shards. Tokio's LIFO slot remains enabled for immediate connection-local I/O wake latency. Low-density HTTP/TLS lanes rely on socket readiness and the Tokio cooperative budget: explicit HTTP yields are amortized over 256 responses until connections reach 64 per data worker, then use the 32-response saturated fairness boundary that preserved 2x mixed throughput. WebSocket relays use the normal 64-poll readiness-bounded turn instead of creating short-period self-wakes. The global injection queue remains at 31 polls, while the I/O driver is checked every 8 polls to reduce wake latency without the throughput cost previously observed from a 15-poll global queue. HTTP/2 uses fixed large windows and the bounded static cache directly; do not add a same-path global snapshot that becomes a shared hot lock. Do not reintroduce separate always-runnable TLS/UDP runtime pools or isolated one-thread runtimes that preserve SO_REUSEPORT hash skew under mixed load. Per-request metrics must use shard-local padded counters instead of one cross-core cache line. Small-profile sendfile keeps a 2 MiB cooperative slice; a 16 MiB ceiling was rejected because it starved mixed HTTP siblings on shared shards. `balanced` may share UDP while retaining its bounded TLS runtime, and `bulk` may use dedicated transfer owners.
...
- One-second equal-offered-load plans must remain mathematically executable: HTTP/SSE workers use centered uniform phase offsets, the gate derives the rounded integer aggregate schedule, and a worker waiting beyond the measurement deadline exits immediately instead of extending process grace.

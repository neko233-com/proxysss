...
- Balanced Linux TLS scheduling uses an independent `ceil(cpuset cores / 3)` crypto runtime at nice `+6`: the worker count must keep scaling with the effective cpuset while bounding aggregate TLS competition with the per-core plain HTTP/UDP/realtime shards. Any change to this share must re-run the strict mixed saturation and equal-load latency matrix; isolated HTTPS wins are insufficient.

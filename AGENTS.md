...
- The default `small` traffic profile keeps HTTP/1.1, HTTP/2/TLS, WebSocket, TCP, UDP, and transparent QCP on one CPU-sized Tokio data runtime with one worker per allowed core and work stealing across those per-core shards. Tokio's LIFO slot remains enabled for immediate connection-local I/O wake latency. Low-density HTTP/TLS lanes amortize explicit yields over 256 responses; high-density plain HTTP uses a 128-response fairness boundary so 4x mixed throughput is not consumed by scheduler bookkeeping. Static file revalidation is a separate 256-hit concern and must never inherit the scheduler fairness batch. WebSocket relays use the normal 64-poll readiness-bounded turn. The global injection queue remains at 31 polls and the I/O driver at 16 polls; the 8-poll experiment reduced 4x HTTP throughput without fixing tail latency. HTTP/2 uses fixed large windows plus a warm-up-built immutable ArcSwap/FxHashMap response snapshot; do not put same-path H2 requests behind a DashMap shard or populate the snapshot concurrently on the first live requests. Do not reintroduce separate always-runnable TLS/UDP runtime pools or isolated one-thread runtimes. Per-request metrics must use shard-local padded counters instead of one cross-core cache line. Small-profile sendfile keeps a 2 MiB cooperative slice; a 16 MiB ceiling was rejected because it starved mixed HTTP siblings on shared shards. `balanced` may share UDP while retaining its bounded TLS runtime, and `bulk` may use dedicated transfer owners.

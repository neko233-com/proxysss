...
- Balanced Linux connection-local runtimes amortize global-queue checks over 31 task polls but drive I/O/timers every 8 polls to protect mixed HTTPS/realtime tails. Plain HTTP cached bodies larger than the pooled single-write limit use one vectored header/body write followed by bounded async remainder writes; do not reintroduce per-response `TCP_CORK` toggles or a separate header syscall for this path without full mixed regression evidence.

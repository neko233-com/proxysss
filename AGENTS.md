...
- One-second equal-offered-load plans must remain mathematically executable: HTTP/SSE workers are uniformly phase-spread from offset zero, the gate derives the exact integer aggregate schedule, and a worker waiting beyond the measurement deadline exits immediately instead of extending process grace.

...
- Balanced Linux connection-local runtimes amortize global-queue checks over 31 task polls. Plain HTTP/UDP shards drive I/O/timers every 8 polls, while the independent TLS/H2 runtime drives them every 4 polls so a multiplexed H2 connection does not wait behind a long connection-task batch. The TLS static fast lane sends the response header and immutable cached body through rustls' vectored writer, then completes any remainder with async backpressure; do not rebuild a contiguous response buffer and copy the cached body on every request.

...
for value in CONNECTIONS CAPACITY_CONNECTIONS DURATION_SECS CAPACITY_HOLD_SECS CAPACITY_CONNECT_WORKERS NOFILE_LIMIT NGINX_WORKERS GATEWAY_PORT BACKEND_PORT; do
...
gateway_pid=""
stop_processes() {
  if [[ -n "$gateway_pid" ]]; then ssh_run "$GATEWAY_HOST" "kill '$gateway_pid' 2>/dev/null || true" || true; fi
...
start_gateway() {
  ...
  if [[ "$kind" == proxysss ]]; then
    gateway_pid="$(ssh_run "$GATEWAY_HOST" "cd '$REMOTE_ROOT' && ulimit -n '$NOFILE_LIMIT' && nohup ./proxysss -config proxysss.yaml >proxysss.log 2>&1 & echo \$!")"
  else
    gateway_pid="$(ssh_run "$GATEWAY_HOST" "cd '$REMOTE_ROOT' && ulimit -n '$NOFILE_LIMIT' && nohup '$NGINX_BIN' -c nginx.conf -g 'daemon off;' >nginx.log 2>&1 & echo \$!")"
  fi
  [[ "$gateway_pid" =~ ^[0-9]+$ ]] || { echo "$kind failed to start" >&2; exit 1; }
  ...
  ssh_run "$GATEWAY_HOST" "cat '$REMOTE_ROOT/${kind}.log' 2>/dev/null || true" >&2 || true
  exit 1
}
stop_gateway() {
  ssh_run "$GATEWAY_HOST" "kill '$gateway_pid' 2>/dev/null || true"
  for _ in $(seq 1 50); do
    if ! ssh_run "$GATEWAY_HOST" "kill -0 '$gateway_pid' 2>/dev/null"; then break; fi
    sleep 0.1
  done
  gateway_pid=""
}
...
run_capacity() {
  ...
  start_gateway "$kind"
  "$PROXY_BIN" bench websocket --url "wss://${GATEWAY_ADDR}:${GATEWAY_PORT}/ws/" --connections "$CAPACITY_CONNECTIONS" --hold-connections --connect-workers "$CAPACITY_CONNECT_WORKERS" --duration-secs "$CAPACITY_HOLD_SECS" --insecure | tee "$output"
  stop_gateway
...
scp "${SSH_ARGS[@]}" "$GATEWAY_HOST:$REMOTE_ROOT/proxysss.log" "$RUN_DIR/gateway-proxysss.log" 2>/dev/null || true
scp "${SSH_ARGS[@]}" "$GATEWAY_HOST:$REMOTE_ROOT/nginx.log" "$RUN_DIR/gateway-nginx.log" 2>/dev/null || true
...
method=separate-linux-hosts-balanced-order-median-saturation-equal-load-strict-p50-p95-p99-zero-errors-capacity
EOF

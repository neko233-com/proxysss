...
impl AliyunCnDns {
    pub fn new(credentials: &BTreeMap<String, String>) -> Result<Self> {
        Ok(Self {
            client: Client::new(),
            access_key_id: credential(credentials, &["access_key_id", "AccessKeyId", "Ali_Key"])?,
            access_key_secret: credential(
                credentials,
                &["access_key_secret", "AccessKeySecret", "Ali_Secret"],
            )?,
        })
    }
    ...
    async fn call(&self, action: &str, params: BTreeMap<String, String>) -> Result<AliyunResponse> {
        ...
        let response = self
            ...
            .send()
            .await
            .with_context(|| format!("aliyun_cn {action} request failed"))?
            .error_for_status()
            .with_context(|| format!("aliyun_cn {action} returned error status"))?
            .json::<AliyunResponse>()
            .await
            .with_context(|| format!("aliyun_cn {action} response decode failed"))?;
        if let Some(code) = &response.code {
            if code != "DomainRecordDuplicate" {
                return Err(anyhow!(
                    "aliyun_cn {action} failed: {} ({})",
    ...
    pub async fn upsert_txt_record(&self, fqdn: &str, value: &str) -> Result<DnsRecordHandle> {
        ...
        let record_id = response
            .record_id
            .ok_or_else(|| anyhow!("aliyun_cn AddDomainRecord returned empty RecordId"))?;

        Ok(DnsRecordHandle {
            provider: "aliyun_cn".to_string(),
            record_id,
            zone: domain_name,
            name: rr,
        })

...
// Data-plane shards mostly run connection-local tasks. Amortize the global
// injection queue over 31 polls, but drive I/O/timers every eight polls so a
...
impl Gateway {
    ...
    async fn serve_admin_api(
        ...
        if method == Method::GET && (path == "/docs" || path == "/docs.html") {
            return Ok(html_response(
                StatusCode::OK,
                render_docs_html(&state.config),
            ));
        }
    ...
    Stream: AsyncWrite + Unpin,
{
    ...
    if written < header.len() {
        ...
        stream.write_all(&body[written - header.len()..]).await
    }
}
...
pub(crate) fn render_docs_html(_config: &GatewayConfig) -> String {
    let mut html = include_str!("../templates/docs.html")
        .replace("__DOCS_STYLE__", include_str!("../docs/site.css"))
        .replace("__DOCS_SCRIPT__", include_str!("../docs/site.js"));
...
fn render_admin_console_html(config: &GatewayConfig) -> String {
    let html = include_str!("../templates/admin.html")
        .replace("__ADMIN_STYLE__", include_str!("../templates/admin.css"))
        .replace(
            "__ADMIN_WRITE_OPS__",
            if config.admin.enable_write_ops {
                "true"
    ...
    html.replace("__ADMIN_USER__", &html_escape(&config.admin.username))
        .replace(
            "__ADMIN_HTTPS_HINT__",
            &if config.admin.https.enabled {
                format!(
                    "HTTPS 管理 API：<strong>https://&lt;host&gt;{}/v1/*</strong>（需要 TLS；证书就绪后可写入）",
                    html_escape(&crate::config::normalize_admin_https_path_prefix(&config.admin.https.path_prefix))
                )
            } else {
                "证书就绪后，可开启 <code>admin.https.enabled</code>，通过 HTTPS 进行远程管理。".to_string()
            },
        )
}
...
mod tests {
    ...
    async fn vectored_static_response_preserves_header_and_large_body() {
        ...
        let addr = listener.local_addr().expect("vectored fixture address");
        ...
        let mut client = TcpStream::connect(addr)
            .await
            .expect("connect static fixture");
        let mut received = Vec::with_capacity(header.len() + body_len);
        client
            .read_to_end(&mut received)
            .await
            .expect("read vectored static response");
        sender
            .await
            .expect("join vectored static fixture")
            .expect("write vectored static response");
    ...
    fn welcome_page_stays_brand_focused() {
        ...
        assert!(html.contains(">GitHub <span"));
        assert!(html.contains(">GitHub Docs <span"));
    ...
    fn builtin_docs_explain_free_acme_key_compatibility() {
        let mut config = GatewayConfig::default();
        config.admin.bearer_token = "private-docs-regression-token".to_string();
        let html = render_docs_html(&config);
        assert!(html.contains("ECDSA P-256"));
        assert!(html.contains("RSA-2048"));
        assert!(html.contains("免费"));
        assert!(!html.contains("private-docs-regression-token"));
        assert!(!html.contains("__DOCS_"));
    ...
    fn admin_console_uses_login_screen_without_embedding_password() {
        ...
        assert!(html.contains("data-write-ops=\"false\""));

        config.admin.username = "\" autofocus onfocus=\"alert(1)".to_string();
        config.admin.enable_write_ops = true;
        let html = render_admin_console_html(&config);
        assert!(html.contains("value=\"&quot; autofocus onfocus=&quot;alert(1)\""));
        assert!(html.contains("data-write-ops=\"true\""));
        assert!(!html.contains("value=\"\" autofocus"));
    }

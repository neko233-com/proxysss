...
function classifyPrivate(ip: string): GeoRule | undefined {
  if (ip === "127.0.0.1" || ip === "::1") {
    return { country: "Loopback", country_code: "LO", city: "localhost", source: "builtin", confidence: "high" };
  }
  if (ip.startsWith("10.") || ip.startsWith("192.168.") || /^172\.(1[6-9]|2\d|3[0-1])\./.test(ip)) {
    return { country: "Private Network", country_code: "PR", city: "private", source: "builtin", confidence: "medium" };
  }
  if (ip.startsWith("169.254.") || ip.startsWith("fe80:")) {
    return { country: "Link Local", country_code: "LL", city: "link-local", source: "builtin", confidence: "medium" };
  }
  return undefined;
}
...
function resolveGeo(ip: string): GeoRule {
  ...
  return { country: "Unknown", country_code: "ZZ", source: "unmapped", confidence: "low" };
}

...
- Bounded the default-small TLS and native realtime runtimes to one owner per four detected CPUs, and its UDP/QCP runtime to one owner per two CPUs. Full-CPU `SO_REUSEPORT` listener fanout and adaptive scaling remain intact while mixed-traffic scheduler contention falls.

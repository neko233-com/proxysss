...
- Added a Linux native `epoll` lane for policy-validated exact static keep-alive traffic: combined small responses use one `send`, large responses use `TCP_CORK` plus `sendfile`, and configuration/file/request changes transfer the socket and unread bytes back to Tokio/Hyper without semantic drift.
